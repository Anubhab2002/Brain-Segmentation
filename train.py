...
import monai
from monai.data import create_test_image_3d, list_data_collate, pad_list_data_collate, decollate_batch
...
def main():
    ...
    # train images
    # image_folder = '/home/anu2002/projects/def-uofavis-ab/shared_data/Diffusion_MRI/subjects'
    # train_images = []
    # train_segs = []
    # val_images = []
    # val_segs = []
    ...
    #     # print(folder)
    #     if folder.startswith('cbbrain') and (folder.split('_')[1]<'138'):
    #         train_images.append(os.path.join(image_folder, folder, 'mean_dwi.nii.gz'))
    #         train_segs.append(os.path.join(image_folder, folder, 'whole_roi.nii'))
    #     if folder.startswith('cbbrain') and (folder.split('_')[1]>'138' and folder.split('_')[1]<'158'):
    #         val_images.append(os.path.join(image_folder, folder, 'mean_dwi.nii.gz'))
    #         val_segs.append(os.path.join(image_folder, folder, 'whole_roi.nii'))
    #     if folder.startswith('ab300') and (folder.split('_')[1]>'000' and folder.split('_')[1]<'003'):
    #         val_images.append(os.path.join(image_folder, folder, 'mean_dwi.nii.gz'))
    #         val_segs.append(os.path.join(image_folder, folder, 'whole_roi.nii'))
    #     # if folder.startswith('ab300') and (folder.split('_')[1]>'499' and folder.split('_')[1]<'503'):
    #     #     val_images.append(os.path.join(image_folder, folder, 'mean_dwi.nii.gz'))
    #     #     val_segs.append(os.path.join(image_folder, folder, 'whole_roi.nii.gz'))
    #     # if folder.startswith('ab300') and (folder.split('_')[1]>'002' and folder.split('_')[1]<'500'):
    #     #     test_images.append(os.path.join(image_folder, folder, 'mean_dwi.nii.gz'))
        

    train_images = sorted(glob(os.path.join(opt.images_folder, 'train', 'image*.nii.gz')))
    ...
    train_images_for_dice = sorted(glob(os.path.join(opt.images_folder, 'train', 'image*.nii.gz')))
    train_segs_for_dice = sorted(glob(os.path.join(opt.labels_folder, 'train', 'label*.nii')))
    # train_images_for_dice = train_images
    # train_segs_for_dice = train_segs

    # test_images = val_images
    # test_segs = val_segs

    # validation images
    val_images = sorted(glob(os.path.join(opt.images_folder, 'val', 'image*.nii.gz')))
    val_segs = sorted(glob(os.path.join(opt.labels_folder, 'val', 'label*.nii')))

    # test images
    test_images = sorted(glob(os.path.join(opt.images_folder, 'test', 'image*.nii.gz')))
    ...
    test_dicts = [{'image': image_name, 'label': label_name} for image_name, label_name in zip(test_images, test_segs)]
    ...
    train_loader = DataLoader(check_train, batch_size=opt.batch_size, shuffle=True, collate_fn=pad_list_data_collate, num_workers=opt.workers, pin_memory=False)

    # create a training_dice data loader
    check_val = monai.data.Dataset(data=train_dice_dicts, transform=val_transforms)
    train_dice_loader = DataLoader(check_val, batch_size=1, num_workers=opt.workers, collate_fn=pad_list_data_collate, pin_memory=False)

    # create a validation data loader
    check_val = monai.data.Dataset(data=val_dicts, transform=val_transforms)
    val_loader = DataLoader(check_val, batch_size=1, num_workers=opt.workers, collate_fn=pad_list_data_collate, pin_memory=False)

    # create a validation data loader
    check_val = monai.data.Dataset(data=test_dicts, transform=val_transforms)
    test_loader = DataLoader(check_val, batch_size=1, num_workers=opt.workers, collate_fn=pad_list_data_collate, pin_memory=False)


    def norm(tensor):
        # Convert the list to a PyTorch tensor
        converted_tensor = torch.tensor(tensor)*2
        converted_tensor = converted_tensor.tolist()
    ...
    if opt.network is 'nnunet':
        ...
        optim = torch.optim.AdamW(net.parameters(), lr=1e-3, weight_decay=1e-5)
        # scheduler = torch.optim.lr_scheduler.LambdaLR(optim, lr_lambda=lambda epoch: 0.00025*(0.9**epoch))
    ...
    for epoch in range(opt.epochs):
        ...
        # scheduler.step()
        # optim.param_groups[0]['lr'] = optim.param_groups[0]['lr']*0.9
        epoch_loss /= step
            ...
            with torch.no_grad():
                ...
                def plot_dice(images_loader):
                    ...
                    for data in images_loader:
                        ...
                        # val_images, val_labels = data["image"], data["label"]
                        ...
                        # val_outputs = norm(val_outputs)
                        # print(val_outputs, type(val_labels))
                        # print("val labels before: ", val_labels)
                        val_outputs = norm(val_outputs[0])
                        val_outputs = torch.tensor(val_outputs).cuda()
                        val_outputs = val_outputs
                        val_outputs = torch.unsqueeze(val_outputs, dim=0)
                        # print("val labels after: ", val_labels)
